...
window.addEventListener('DOMContentLoaded', function () {
    ...
    function showSkills() { //  when end game and during game
        if (unlockedSkills === 0) {
    ...
    function chrono() { // ONLY when player TWO joins // back or front
        if (startTime) {
            time += 1;
    ...
    function mouseClicked(e) {
        if (e.pageX < player.position.x + player.radius + canvas.offsetLeft &&
            ...
            e.pageY > player.position.y - player.radius + canvas.offsetTop) {
            player.position.x = e.pageX - canvas.offsetLeft - player.radius + player.radius / 2;
            player.position.y = e.pageY - canvas.offsetTop - player.radius + player.radius / 2;
            mouseStatement = true;
            players_ready = true;
